...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (searchForm) {
        searchForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            const clientIdStr = document.getElementById('search-client-id').value;
            const companyName = document.getElementById('search-company-name').value;
            const clientEmail = document.getElementById('search-client-email').value;
...
async function renderInvoices(invoices) {
    ...
    container.innerHTML = invoices.map(invoice => {
        const companyName = clientMap.get(invoice.client_id) || 'Unknown Client';
